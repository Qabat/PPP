...
spectraDelays = [50 60 70 80 90 100];
timeZero = 0;
...
mainFontsize = 20;
legendFontsize = 14;
plusName = ['_' 'submission2' '_' cmap '_' num2str(numColor)];
...
if dechirpSample == 1
    ...
    elseif dechirpType == 2
        [TAmapDechirped, delays] = dechirpClicking(chirpedTAmap, delays, lambdas, delayRange, lambdaRange, [-0.5 0.5], intensityAxis, xAxis, linewidth, mainFontsize, fileLocation);
    end
...
[TAmapDechirped, delays, lambdas] = readMap(fileLocation, '_dechirped');
...
lambda600Decay = TAmapSmoothed(565, timeStart:timeStop);
lambda640Decay = TAmapSmoothed(640, timeStart:timeStop);
% 
% % 
...
fun4 = @(x,xdata) x(1)*exp(x(2)*xdata) + x(3)*exp(x(4)*xdata) + x(5)*exp(x(6)*xdata) + x(7)*exp(x(8)*xdata);
% % 
% % x01 = [0.35, 0.05];
% x02 = [1.53, 8.48e-05, -1.629, -0.002377];
% x03 = [1.53, 8.48e-05, -1.629, -0.002377, 0.1, -0.002];
x04 = [1.53, 8.48e-05, -1.629, -0.002377, 0.1, -0.002, 0.1, -0.002];
% 
% % 
[fitted,resnorm,residual,exitflag,output] = lsqcurvefit(fun4, x04, fitDelays, lambda330Decay);
...
fitfun4 = fitted(1) .* exp(fitted(2).*fitDelays) + fitted(3) .* exp(fitted(4).*fitDelays) + fitted(5) .* exp(fitted(6).*fitDelays) + fitted(7) .* exp(fitted(8).*fitDelays);
% 
figure()
times = linspace(fitDelays(1),fitDelays(end));
hold on
plot(fitDelays, lambda330Decay,'ko');
