...
function [TAmap, cutScans, newlambdas] = rejectScansSpectra(allScans, lambdas, delays, lambdaRange, delayRange, fileLocation)
...
%     hold off
    ...
    dotMatrix = zeros(scanNumber-1, length(delays));
...
%                         dotScans = dot(cutScans(kk,:,1), cutScans(kk,:,ii));
    ...
    subplot(2,1,1)
    pcolor(scanAxis, delays, dotMatrix);
    intensityRange = [0.8 1];
