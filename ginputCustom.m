function [out1,out2,out3] = ginputCustom(arg1, pointerColor)
...
if ~matlab.ui.internal.isFigureShowEnabled
    ...
    initialState = setupFcn(fig, pointerColor);
...
function initialState = setupFcn(fig, pointerColor)
...
initialState.CrossHair = createCrossHair(fig, pointerColor);
...
gap = 1; % 1 pixel view port between the crosshairs
...
function crossHair = createCrossHair(fig, pointerColor)
...
for k = 1:4
    crossHair(k) = uicontrol(fig, 'Style', 'text', 'Visible', 'off', 'Units', 'pixels', 'BackgroundColor', pointerColor, 'HandleVisibility', 'off', 'HitTest', 'off'); %#ok<AGROW>
end
end
